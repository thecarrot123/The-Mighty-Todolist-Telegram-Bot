...
from app.bot import start_command, help_command, init_db, add_task, notify_due_tasks, list_tasks, delete_task, mark_completed
import sqlite3
...
async def test_add_task_valid_input():
    ...
    with patch("sqlite3.connect") as mock_connect:
        ...
        update.message.reply_text.assert_called_with("Task added successfully!")

@pytest.mark.asyncio
async def test_notify_due_tasks():
    with patch("sqlite3.connect") as mock_connect, \
         patch("app.bot.Bot.send_message") as mock_send_message:
        mock_cursor = Mock()
        mock_connect.return_value.cursor.return_value = mock_cursor
        mock_cursor.fetchall.return_value = [(1, 12345, "Prepare meeting")]
        await notify_due_tasks()
        mock_send_message.assert_called_with(chat_id=12345, text="Reminder: Task 'Prepare meeting' is due in 24 hours!")
...
async def test_mark_completed_success():
    ...
    with patch('sqlite3.connect') as mock_connect:
        mock_connection = mock_connect.return_value
        mock_cursor = mock_connection.cursor.return_value
        mock_cursor.fetchone.return_value = [42]  # Task exists and is not completed
        
        await mark_completed(update, context)
        
        update.message.reply_text.assert_awaited_once_with("Task marked as completed successfully!")
        mock_cursor.execute.assert_any_call(
            "UPDATE tasks SET completed = TRUE WHERE id = ? AND user_id = ?", (42, 12345)
        )
...
async def test_mark_completed_not_found():
    ...
    with patch('sqlite3.connect') as mock_connect:
        mock_connection = mock_connect.return_value
        mock_cursor = mock_connection.cursor.return_value
        mock_cursor.fetchone.return_value = None  # Task does not exist or already completed
        
        await mark_completed(update, context)
        
        update.message.reply_text.assert_awaited_once_with("Task not found or already completed.")
...
async def test_mark_completed_db_error():
    ...
    with patch('sqlite3.connect') as mock_connect:
        ...
        update.message.reply_text.assert_awaited_once_with("Failed to complete task due to a database error.")
...
async def test_delete_task_success():
    ...
    with patch('sqlite3.connect') as mock_connect:
        ...
        update.message.reply_text.assert_awaited_once_with("Task deleted successfully!")
...
async def test_delete_task_not_found():
    ...
    with patch('sqlite3.connect') as mock_connect:
        ...
        update.message.reply_text.assert_awaited_once_with("Task not found or does not belong to you.")
...
async def test_delete_task_db_error():
    ...
    with patch('sqlite3.connect') as mock_connect:
        ...
        update.message.reply_text.assert_awaited_once_with("Failed to delete task due to a database error.")
